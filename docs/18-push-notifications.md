...
- [ ] Implement permission prompt with clear value proposition; handle denial gracefully.
- [ ] Register and store Expo push token (or APNs/FCM token) tied to the authenticated user/service provider.
- [ ] Implement server-side trigger (Supabase function or backend) to send notifications for chosen events.
- [ ] Add in-app handlers for foreground notifications and deep links into screens (inquiry detail, assistant settings).
